...
| `actorfacialoverridemotion` | ActorFacialOverrideMotionEditor | 模式三 + 动态列表 + JSON转义 | ✅ 完成 |
| `voice` | VoiceEditor | 模式一 + DB API | ✅ 完成 |
...
| `bgmplay` | BgmPlayEditor | 模式一 + FormEditor | ✅ 完成 |
| `bgmstop` | BgmStopEditor | 模式一（简化） | ✅ 完成 |
| `actoradditivemotion` | ActorAdditiveMotionEditor | 复用 ActorResourceEditor | ✅ 完成 |
...
| 🟡 | **Shake** | 中 | 极低 | 中 | 2 |
| 🟡 | **Se** | 中 | 极低 | 中 | 3 |
| 🟢 | **ActorEyeBlink** | 低 | 极低 | 低 | 4 |
| 🟢 | **ActorLookTarget** | 低 | 低 | 低 | 5 |
| 🟢 | **Dof** | 低 | 低 | 低 | 6 |
| 🟢 | **ColorEffect** | 低 | 低 | 低 | 7 |
| 🟢 | **ActorLookTargetTween** | 低 | 低 | 低 | 8 |
| 🟢 | **ActorLighting** | 低 | 中 | 低 | 9 |
| 🟢 | **Timeline** | 低 | 极低 | 低 | 10 |
...
1. **Shake** - 仅 2 个数字字段
2. **Se** - 仅 2 个字段
3. **ActorEyeBlink** - 仅 2 个字段
4. **Dof** - 仅 3 个数字字段
5. **ColorEffect** - 仅 2 个字段
6. **Timeline** - 仅 1 个文本字段

**预计减少：** 约 6 个命令使用 GenericEditor → 专用编辑器
