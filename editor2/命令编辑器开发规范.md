# 命令编辑器开发规范

> 📋 本文档是 Editor2 项目的编辑器开发规范，整合了所有重构经验和最佳实践。  
> 📅 创建时间: 2026-01-13  
> ✅ 状态: 基于阶段1-3完整重构经验编写

---

## 目录

1. [架构概述](#架构概述)
2. [基础设施使用指南](#基础设施使用指南)
3. [编辑器开发三大模式](#编辑器开发三大模式)
4. [开发流程规范](#开发流程规范)
5. [最佳实践和注意事项](#最佳实践和注意事项)
6. [代码审查清单](#代码审查清单)
7. [常见问题解答](#常见问题解答)

---

## 架构概述

### 设计理念

Editor2 采用模块化、可复用的架构设计：

```
┌─────────────────────────────────────────────────────────┐
│                      App.tsx (主应用)                      │
│  - EditorContext 状态管理                                 │
│  - 卡片列表管理                                           │
│  - 编辑对话框协调                                         │
└─────────────────────────────────────────────────────────┘
                            │
        ┌───────────────────┼───────────────────┐
        ▼                   ▼                   ▼
┌──────────────┐   ┌──────────────┐   ┌──────────────┐
│   Context    │   │    Hooks     │   │  Components  │
│              │   │              │   │              │
│ EditorContext│   │useResourceAPI│   │ LoadingState │
│              │   │useAvailableIds│   │ ErrorState   │
│              │   │              │   │ EmptyState   │
│              │   │              │   │ VectorInput  │
│              │   │              │   │ FormEditor   │
└──────────────┘   └──────────────┘   └──────────────┘
                            │
        ┌───────────────────┼───────────────────┐
        ▼                   ▼                   ▼
┌──────────────┐   ┌──────────────┐   ┌──────────────┐
│ 模式一编辑器  │   │ 模式二编辑器  │   │ 模式三编辑器  │
│ DialogueEditor│   │BackgroundGroup│   │CameraSetting │
│ FadeEditor   │   │ActorGroup    │   │              │
└──────────────┘   └──────────────┘   └──────────────┘
```

### 核心原则

1. **🚫 禁止使用 window 对象** - 全部使用 EditorContext
2. **♻️ 最大化代码复用** - 使用通用 Hooks 和组件
3. **📦 统一数据获取** - 使用 useResourceAPI
4. **🎨 统一 UI 体验** - 使用通用状态组件
5. **✅ 类型安全优先** - 所有接口都有完整的类型定义

---

## 基础设施使用指南

### 1. EditorContext - 状态管理

**❌ 错误做法（禁止）：**
```typescript
// 不要使用 window 对象！
const cards = (window as any).__editorCards;
```

**✅ 正确做法：**
```typescript
import { useEditorContext } from '../../../context/EditorContext';

const YourEditor: React.FC<Props> = ({ card, onChange }) => {
  const { cards } = useEditorContext();
  // 使用 cards
};
```

**可用的 Hooks：**
- `useEditorContext()` - 获取完整的 Context（cards, selectedCard, setSelectedCard）
- `useEditorCards()` - 仅获取 cards 数组
- `useSelectedCard()` - 仅获取选中的卡片

---

### 2. useResourceAPI - 统一 API 调用

**用途：** 从 Database API 加载资源（动作、表情、环境、模型等）

**提供的 Hooks：**

```typescript
// 通用 API 调用
const { data, loading, error, reload } = useResourceAPI<T>(endpoint);

// 预定义的专用 Hooks
const { data: motions, loading, error, reload } = useMotions('character' | 'facial');
const { data: environments, loading, error, reload } = useEnvironments();
const { data: models, loading, error, reload } = useModels(characterId, modelType);
const { data: characters, loading, error, reload } = useCharacters();
```

**标准使用模式：**
```typescript
export const YourEditor: React.FC<Props> = ({ card, onChange }) => {
  const { data: resources, loading, error, reload } = useMotions('character');

  // 1. 显示加载状态
  if (loading) return <LoadingState message="加载资源中..." />;
  
  // 2. 显示错误状态（带重试按钮）
  if (error) return <ErrorState message="加载失败" details={error} onRetry={reload} />;
  
  // 3. 正常渲染
  return (
    <select>
      {resources.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
    </select>
  );
};
```

**优势：**
- ✅ 自动错误处理
- ✅ 统一的重试逻辑
- ✅ 类型安全
- ✅ 易于添加缓存

---

### 3. useAvailableIds - 统一 ID 获取

**用途：** 从已有的 Group 命令中提取可用的 ID 列表

**提供的 Hooks：**

```typescript
const availableActorIds = useAvailableActorIds();
// 返回: string[] - 所有 actorgroup 中定义的角色 ID

const available2DBackgrounds = useAvailableBackgroundIds('2d');
const available3DBackgrounds = useAvailableBackgroundIds('3d');
// 返回: string[] - 所有 backgroundgroup 中定义的背景 ID（根据类型筛选）
```

**标准使用模式：**
```typescript
export const YourEditor: React.FC<Props> = ({ card, onChange }) => {
  const availableActorIds = useAvailableActorIds();
  
  // 检查是否有可用 ID
  if (availableActorIds.length === 0) {
    return (
      <EmptyState 
        title="无可用角色" 
        description="请先添加 actorgroup 命令来定义角色"
      />
    );
  }
  
  // 正常渲染
  return (
    <select>
      {availableActorIds.map(id => <option key={id} value={id}>{id}</option>)}
    </select>
  );
};
```

**2D/3D 背景判断工具：**
```typescript
import { isBackground2D } from '../../../hooks/useAvailableIds';

const is2D = isBackground2D(backgroundSrc); // 根据扩展名判断
```

---

### 4. 通用 UI 组件

#### LoadingState - 加载状态

```typescript
import { LoadingState } from '../../common';

<LoadingState message="加载资源中..." />
```

#### ErrorState - 错误状态（带重试）

```typescript
import { ErrorState } from '../../common';

<ErrorState 
  message="加载失败" 
  details={error} 
  onRetry={reload} 
/>
```

#### EmptyState - 空状态提示

```typescript
import { EmptyState } from '../../common';

<EmptyState 
  title="无可用数据" 
  description="请先添加前置命令"
  icon="📦"  // 可选
/>
```

#### VectorInput - 向量输入

```typescript
import { Vector2Input, Vector3Input } from '../../common';

// 2D 向量
<Vector2Input
  value={{ x: 0, y: 0 }}
  onChange={setPosition}
  step={0.01}
  labels={{ x: 'X轴', y: 'Y轴' }}  // 可选
/>

// 3D 向量
<Vector3Input
  value={{ x: 0, y: 0, z: 0 }}
  onChange={setRotation}
  step={0.1}
  labels={{ x: 'X (度)', y: 'Y (度)', z: 'Z (度)' }}
/>
```

---

## 编辑器开发三大模式

根据命令的复杂度，编辑器分为三种实现模式：

### 📋 模式一：简单参数编辑器（推荐用 FormEditor）

**适用场景：**
- 命令只有简单的文本、数字、下拉选择等参数
- 参数直接存储在 `params` 对象中
- 不涉及嵌套结构或 JSON 序列化

**代表命令：** `dialogue`, `message`, `narration`, `fade`, `transition`

**实现模板：**

```typescript
import React, { useState, useEffect } from 'react';
import { CommandCard } from '../../types/command';
import { FormEditor, FormSection } from '../FormEditor';

interface YourEditorProps {
  card: CommandCard;
  onChange: (updatedCard: CommandCard, isValid?: boolean) => void;
}

export const YourEditor: React.FC<YourEditorProps> = ({ card, onChange }) => {
  const [formData, setFormData] = useState(card);

  // 定义表单结构
  const sections: FormSection[] = [
    {
      title: '基本设置',
      fields: [
        {
          name: 'id',
          label: 'ID',
          type: 'text',
          required: true,
          value: formData.params.id || '',
        },
        {
          name: 'value',
          label: '数值',
          type: 'number',
          value: formData.params.value || 0,
          min: 0,
          max: 100,
        },
      ],
    },
  ];

  // 字段变化处理
  const handleFieldChange = (fieldName: string, value: any) => {
    const updatedCard = {
      ...formData,
      params: {
        ...formData.params,
        [fieldName]: value,
      },
    };
    setFormData(updatedCard);
  };

  // 实时同步到父组件
  useEffect(() => {
    // 验证必填字段
    const isValid = formData.params.id !== undefined && formData.params.id !== '';
    onChange(formData, isValid);
  }, [formData]);

  return <FormEditor sections={sections} onChange={handleFieldChange} />;
};
```

**关键点：**
- ✅ 使用 FormEditor 统一 UI
- ✅ 实时验证和同步（useEffect + onChange）
- ✅ 通过第二个参数 `isValid` 控制保存按钮状态
- ✅ 清晰的字段定义（支持 text, number, select, textarea 等）

**条件参数处理：**

如果某些参数根据其他参数动态显示/隐藏（如 `transition` 命令）：

```typescript
const handleFieldChange = (fieldName: string, value: any) => {
  const updatedParams = { ...formData.params };
  
  // 更新字段
  updatedParams[fieldName] = value;
  
  // 条件逻辑：根据 type 决定是否需要 character
  if (fieldName === 'type') {
    if (value === 'event_change') {
      // event 类型不需要 character
      delete updatedParams.character;
    } else {
      // 非 event 类型需要 character（默认空，等待用户填写）
      if (!updatedParams.character) {
        updatedParams.character = '';
      }
    }
  }
  
  const updatedCard = { ...formData, params: updatedParams };
  setFormData(updatedCard);
};

// 在 sections 中使用 disabled 控制
{
  name: 'character',
  label: '角色',
  type: 'select',
  options: characters,
  value: formData.params.character || '',
  disabled: formData.params.type === 'event_change',  // 条件禁用
  required: formData.params.type !== 'event_change',  // 条件必填
}
```

---

### 🔧 模式二：Group 项编辑器（需要 Database API）

**适用场景：**
- 命令包含多个子项（如多个背景、多个角色）
- 需要单独编辑、添加、删除子项
- 子项格式为 `[itemtype ...]` 的嵌套结构

**代表命令：** `backgroundgroup`, `actorgroup`, `actorlayoutgroup`, `backgroundlayoutgroup`

**架构组成：**

```
1. Renderer（渲染器） - 显示所有子项 + 编辑/添加按钮
2. ItemEditor（子项编辑器） - 编辑单个子项的表单
3. App.tsx 处理逻辑 - 统一的编辑/添加/保存逻辑
```

**完整实现流程：**

#### 步骤 1: 创建子项编辑器

```typescript
// src/components/CommandEditors/BackgroundItemEditor/BackgroundItemEditor.tsx
import React, { useState, useEffect } from 'react';
import { useEnvironments } from '../../../hooks/useResourceAPI';
import { LoadingState, ErrorState } from '../../common';

interface BackgroundItemEditorProps {
  id: string;
  src: string;
  onChange: (data: { id: string; src: string }) => void;
}

export const BackgroundItemEditor: React.FC<BackgroundItemEditorProps> = ({
  id,
  src,
  onChange,
}) => {
  const [localData, setLocalData] = useState({ id, src });
  const { data: environments, loading, error, reload } = useEnvironments();

  // 实时同步到父组件
  useEffect(() => {
    onChange(localData);
  }, [localData]);

  if (loading) return <LoadingState message="加载环境资源..." />;
  if (error) return <ErrorState message="加载失败" details={error} onRetry={reload} />;

  return (
    <div className="background-item-editor">
      <div className="form-field">
        <label>背景ID</label>
        <input
          type="text"
          value={localData.id}
          onChange={(e) => setLocalData({ ...localData, id: e.target.value })}
          placeholder="例如: bg_stage"
        />
      </div>
      <div className="form-field">
        <label>环境资源</label>
        <select
          value={localData.src}
          onChange={(e) => setLocalData({ ...localData, src: e.target.value })}
        >
          <option value="">请选择...</option>
          {environments?.map((env) => (
            <option key={env.id} value={env.src}>
              {env.name}
            </option>
          ))}
        </select>
      </div>
    </div>
  );
};
```

**关键点：**
- ✅ 使用 useResourceAPI 加载资源
- ✅ 显示 LoadingState 和 ErrorState
- ✅ 通过 useEffect 实时同步数据到父组件
- ✅ 验证在父组件（App.tsx）中统一处理

#### 步骤 2: 创建渲染器（支持编辑和添加）

```typescript
// src/renderers/BackgroundGroupRenderer.tsx
import React from 'react';
import { RendererProps } from './index';
import { ParamCard } from './ParamRow';
import { AddItemButton } from './ParamRow';

const BackgroundGroupRenderer: React.FC<RendererProps> = ({ 
  params, 
  onEditItem,   // 编辑某个子项
  onAddItem     // 添加新子项
}) => {
  const backgrounds = parseBackgroundGroup(params);
  
  return (
    <>
      {backgrounds.map((bg, index) => (
        <ParamCard 
          key={index} 
          title="背景设置"
          index={backgrounds.length > 1 ? index + 1 : undefined}  // 多个时显示序号
          onEdit={onEditItem ? () => onEditItem(index) : undefined}
        >
          <ParamRow label="背景ID" value={bg.id} />
          <ParamRow label="环境资源" value={bg.src} />
        </ParamCard>
      ))}
      
      {/* 添加按钮 */}
      {onAddItem && <AddItemButton label="添加背景" onClick={onAddItem} />}
    </>
  );
};

export default BackgroundGroupRenderer;
```

**关键点：**
- ✅ 使用 `onEditItem` 回调处理编辑
- ✅ 使用 `onAddItem` 回调处理添加
- ✅ 使用 `AddItemButton` 统一的添加按钮样式
- ✅ 多个子项时显示序号（`index` prop）

#### 步骤 3: 在 App.tsx 中添加处理逻辑

**3.1 在 `handleEditGroupItem` 中添加类型：**

```typescript
const handleEditGroupItem = (card: CommandCard, itemIndex: number, itemType: string) => {
  const { parseBackgroundGroup, parseActorGroup } = require('./renderers/parserHelpers');
  
  if (itemType === 'background') {
    const backgrounds = parseBackgroundGroup(card.params);
    if (itemIndex >= 0 && itemIndex < backgrounds.length) {
      const bg = backgrounds[itemIndex];
      setEditingCard(card);
      setEditingItemIndex(itemIndex);
      setEditingItemData({ id: bg.id || '', src: bg.src || '' });
    }
  } 
  // else if (itemType === 'actor') { ... }
  // 为你的新类型添加类似的逻辑
};
```

**3.2 在 `handleAddGroupItem` 中添加默认值：**

```typescript
const handleAddGroupItem = (card: CommandCard, itemType: string) => {
  setEditingCard(card);
  setEditingItemIndex(-2);  // -2 表示添加模式
  
  if (itemType === 'background') {
    setEditingItemData({ id: '', src: '' });
  }
  // else if (itemType === 'actor') { ... }
};
```

**3.3 在 `handleSaveGroupItem` 中添加保存逻辑：**

```typescript
const handleSaveGroupItem = () => {
  if (!editingCard) return;
  
  if (editingCard.type === 'backgroundgroup') {
    saveBackgroundGroupItem();
  }
  // else if (editingCard.type === 'actorgroup') { ... }
};

const saveBackgroundGroupItem = () => {
  const { parseBackgroundGroup } = require('./renderers/parserHelpers');
  const backgrounds = parseBackgroundGroup(editingCard.params);
  
  // 添加或更新
  if (editingItemIndex === -2) {
    backgrounds.push(editingItemData);  // 添加新项
  } else if (editingItemIndex >= 0) {
    backgrounds[editingItemIndex] = editingItemData;  // 更新现有项
  }
  
  // 重建命令文本
  const backgroundsText = backgrounds
    .map(bg => `[background id=${bg.id} src=${bg.src}]`)
    .join(' ');
  
  const newRawLine = `[backgroundgroup backgrounds=${backgroundsText}]`;
  
  // 更新卡片
  const updatedCard = {
    ...editingCard,
    raw_line: newRawLine,
    params: { backgrounds: backgroundsText },
    title: generateCardTitle(editingCard.type, { backgrounds: backgroundsText }),
  };
  
  // 更新 cards 列表
  const newCards = cards.map(c => c.id === updatedCard.id ? updatedCard : c);
  setCards(newCards);
  
  // 清理状态
  setEditingCard(null);
  setEditingItemIndex(-1);
  setEditingItemData(null);
};
```

**3.4 在 `renderCommandDetails` 中添加回调：**

```typescript
if (card.type === 'backgroundgroup') {
  props.onEditItem = (index: number) => handleEditGroupItem(card, index, 'background');
  props.onAddItem = () => handleAddGroupItem(card, 'background');
}
// else if (card.type === 'actorgroup') { ... }
```

**3.5 在 `getItemTypeName` 中添加显示名称：**

```typescript
const getItemTypeName = (commandType: string): string => {
  const typeNames: Record<string, string> = {
    'backgroundgroup': '背景设置',
    'actorgroup': '角色设置',
    // 为你的新类型添加
  };
  return typeNames[commandType] || '项';
};
```

**3.6 在 `renderGroupItemEditor` 中添加编辑器：**

```typescript
const renderGroupItemEditor = (commandType: string) => {
  if (commandType === 'backgroundgroup') {
    return (
      <BackgroundItemEditor
        id={editingItemData.id || ''}
        src={editingItemData.src || ''}
        onChange={(data) => setEditingItemData(data)}
      />
    );
  }
  // else if (commandType === 'actorgroup') { ... }
  return null;
};
```

---

### 🎯 模式三：复杂 JSON 参数编辑器

**适用场景：**
- 参数是复杂的嵌套 JSON 结构
- 需要 JSON 序列化/反序列化
- 可能需要转义处理（如 `\{...\}`）

**代表命令：** `camerasetting`, `backgroundsetting`, `actorfacialoverridemotion`

**实现模板：**

```typescript
import React, { useState, useEffect } from 'react';
import { CommandCard } from '../../types/command';
import { Vector3Input } from '../common';

interface YourEditorProps {
  card: CommandCard;
  onChange: (updatedCard: CommandCard, isValid?: boolean) => void;
}

export const YourEditor: React.FC<YourEditorProps> = ({ card, onChange }) => {
  // 解析现有的 JSON 参数
  const parseJsonParam = (paramValue: string | undefined): any => {
    if (!paramValue) return { /* 默认值 */ };
    try {
      // 处理转义的 JSON（如果需要）
      const unescaped = paramValue.replace(/\\{/g, '{').replace(/\\}/g, '}');
      return JSON.parse(unescaped);
    } catch {
      return { /* 默认值 */ };
    }
  };

  const [setting, setSetting] = useState(parseJsonParam(card.params.setting));

  // 序列化并同步到父组件
  useEffect(() => {
    const jsonString = JSON.stringify(setting);
    // 如果需要转义
    const escaped = jsonString.replace(/{/g, '\\{').replace(/}/g, '\\}');
    
    const updatedCard = {
      ...card,
      params: {
        ...card.params,
        setting: escaped,
      },
    };
    onChange(updatedCard, true);
  }, [setting]);

  return (
    <div className="your-editor">
      <Vector3Input
        value={setting.position || { x: 0, y: 0, z: 0 }}
        onChange={(pos) => setSetting({ ...setting, position: pos })}
        labels={{ x: 'X轴', y: 'Y轴', z: 'Z轴' }}
      />
      {/* 其他字段 */}
    </div>
  );
};
```

**关键点：**
- ✅ 使用 try-catch 安全解析 JSON
- ✅ 提供合理的默认值
- ✅ 根据需要处理转义（`\{` → `{`，序列化时 `{` → `\{`）
- ✅ 使用 Vector3Input 等专用组件
- ✅ 实时序列化并同步
- ✅ 动态数组管理（如 faceModels 的添加/删除/编辑）

**特殊案例 - ActorFacialOverrideMotion：**

这个编辑器展示了模式三的高级用法：

```typescript
// 1. 双向 JSON 转义处理
// 解析：\{ → {
const settingStr = params.setting.replace(/\\{/g, '{').replace(/\\}/g, '}');
const parsed = JSON.parse(settingStr);

// 序列化：{ → \{
const settingJson = JSON.stringify(setting);
const escaped = settingJson.replace(/{/g, '\\{').replace(/}/g, '\\}');

// 2. 动态数组管理
const handleAddFaceModel = () => {
  setSetting({
    ...setting,
    faceModels: [...setting.faceModels, { path: 'Root_Face', index: 0, value: 1.0 }]
  });
};

// 3. Path 字段固定处理（始终为 Root_Face）
// 在 UI 中隐藏 path 输入框，简化用户操作

// 4. Value 范围验证（-1.0 到 1.0）
<input type="number" min="-1" max="1" step="0.1" />
```

**实现要点：**
- 固定值字段无需暴露给用户（如 path 始终为 'Root_Face'）
- 负值支持：某些参数支持负值表示反向效果（如 value: -1.0 禁用眨眼）
- 添加 Wiki 链接：帮助用户理解复杂参数（如表情 Index 参考表）

---

## 开发流程规范

### 新增编辑器的完整流程

#### 1. 分析命令类型

**问题清单：**
- [ ] 命令有哪些参数？
- [ ] 参数是简单类型还是复杂类型（JSON）？
- [ ] 是否需要从 Database API 获取资源？
- [ ] 是否需要从其他命令获取 ID 列表？
- [ ] 是否是 Group 类型命令？
- [ ] 是否需要向量输入？

**根据答案选择模式：**
- 简单参数 + 无外部依赖 → 模式一（简单）
- 简单参数 + 需要 API/ID → 模式一（高级）
- Group 命令 → 模式二
- 复杂 JSON 参数 → 模式三

#### 2. 创建编辑器文件

```bash
# 模式一
src/components/CommandEditors/YourEditor/YourEditor.tsx

# 模式二（需要两个文件）
src/components/CommandEditors/YourItemEditor/YourItemEditor.tsx
src/renderers/YourGroupRenderer.tsx
```

#### 3. 实现编辑器逻辑

**使用基础设施：**
- [ ] 使用 `useEditorContext` 替代 window 对象
- [ ] 使用 `useResourceAPI` 加载资源
- [ ] 使用 `useAvailableIds` 获取 ID 列表
- [ ] 使用 `LoadingState` / `ErrorState` / `EmptyState`
- [ ] 使用 `Vector2Input` / `Vector3Input`（如果需要）

**验证逻辑：**
- [ ] 通过 `onChange(card, isValid)` 控制保存按钮
- [ ] 必填字段不能为空
- [ ] 数值范围验证
- [ ] 条件必填（如 transition 的 character）

#### 4. 更新 App.tsx

**模式一：**
- [ ] 在 `renderEditorForType` 中添加 case

**模式二：**
- [ ] 在 `handleEditGroupItem` 中添加解析逻辑
- [ ] 在 `handleAddGroupItem` 中添加默认值
- [ ] 在 `handleSaveGroupItem` 中添加保存逻辑
- [ ] 在 `renderCommandDetails` 中添加回调
- [ ] 在 `getItemTypeName` 中添加名称
- [ ] 在 `renderGroupItemEditor` 中添加编辑器

#### 5. 创建渲染器

```bash
src/renderers/YourRenderer.tsx
```

**必须实现：**
- [ ] 显示所有参数
- [ ] 使用 `ParamCard` 和 `ParamRow`
- [ ] 提供 `onEdit` 回调（模式一）或 `onEditItem` / `onAddItem`（模式二）
- [ ] 条件渲染（如 event 类型隐藏 character）

#### 6. 更新卡片标题

在 `command-card.ts` 的 `generateCardTitle` 中添加 case：

```typescript
case 'yourcommand':
  return `命令标题: ${params.key1} - ${params.key2}`;
```

#### 7. 测试

**必测项目：**
- [ ] 加载现有命令正常显示
- [ ] 编辑参数后保存成功
- [ ] 原始命令文本正确更新
- [ ] 卡片标题正确更新
- [ ] 加载状态正常显示
- [ ] 错误状态 + 重试按钮正常工作
- [ ] 空状态正常显示（如果适用）
- [ ] 验证逻辑正确（保存按钮禁用/启用）
- [ ] 条件参数正确处理（如果适用）

---

## 最佳实践和注意事项

### ✅ 应该做的

1. **使用基础设施**
   - ✅ 所有 API 调用使用 `useResourceAPI`
   - ✅ 所有 ID 获取使用 `useAvailableIds`
   - ✅ 所有状态显示使用通用组件
   - ✅ 所有向量输入使用 `VectorInput`

2. **状态管理**
   - ✅ 使用 `useState` 管理本地表单数据
   - ✅ 使用 `useEffect` 实时同步到父组件
   - ✅ 通过 `onChange(card, isValid)` 控制保存按钮

3. **类型安全**
   - ✅ 为所有组件定义 Props 接口
   - ✅ 使用 TypeScript 类型推导
   - ✅ 避免使用 `any`

4. **用户体验**
   - ✅ 显示清晰的加载状态
   - ✅ 显示友好的错误信息
   - ✅ 提供重试按钮
   - ✅ 显示有意义的空状态提示

5. **代码组织**
   - ✅ 一个编辑器一个文件夹
   - ✅ 使用清晰的文件命名
   - ✅ 导出 index.ts 统一接口

### ❌ 不应该做的

1. **禁止的模式**
   - ❌ 使用 `window` 对象传递数据
   - ❌ 直接修改 props（始终创建新对象）
   - ❌ 在编辑器中直接调用 `setCards`
   - ❌ 硬编码 API URL（使用 `useResourceAPI`）

2. **避免重复**
   - ❌ 复制粘贴现有编辑器代码
   - ❌ 重新实现已有的 API 调用逻辑
   - ❌ 重新实现已有的向量输入 UI
   - ❌ 重新实现已有的加载/错误状态

3. **不好的实践**
   - ❌ 忽略错误处理
   - ❌ 忽略加载状态
   - ❌ 忽略验证逻辑
   - ❌ 忽略条件参数的处理

### ⚠️ 常见陷阱

1. **条件参数处理**
   ```typescript
   // ❌ 错误：只设置值，不删除不需要的参数
   if (type === 'event') {
     updatedParams.character = '';  // 错误：应该删除
   }
   
   // ✅ 正确
   if (type === 'event') {
     delete updatedParams.character;
   } else {
     updatedParams.character = updatedParams.character || '';
   }
   ```

2. **JSON 转义处理**
   ```typescript
   // ❌ 错误：忘记转义
   params.setting = JSON.stringify(setting);
   
   // ✅ 正确
   const escaped = JSON.stringify(setting)
     .replace(/{/g, '\\{')
     .replace(/}/g, '\\}');
   params.setting = escaped;
   ```

3. **验证时机**
   ```typescript
   // ❌ 错误：只在保存时验证
   const handleSave = () => {
     if (!formData.id) {
       alert('ID 不能为空');  // 太晚了！
     }
   };
   
   // ✅ 正确：实时验证
   useEffect(() => {
     const isValid = formData.id !== '' && formData.value > 0;
     onChange(formData, isValid);  // 控制保存按钮
   }, [formData]);
   ```

4. **依赖验证**
   ```typescript
   // ❌ 错误：降级到文本输入（用户体验差）
   if (availableIds.length === 0) {
     return <input type="text" />;  // 不好
   }
   
   // ✅ 正确：显示清晰的提示
   if (availableIds.length === 0) {
     return (
       <EmptyState 
         title="无可用角色" 
         description="请先添加 actorgroup 命令"
       />
     );
   }
   ```

---

## 代码审查清单

### 提交前自查

#### 基础检查
- [ ] 代码没有 TypeScript 错误
- [ ] 没有使用 `window` 对象
- [ ] 使用了适当的基础设施（Hooks、组件）
- [ ] 所有导入路径正确
- [ ] 文件命名符合规范

#### 功能检查
- [ ] 编辑器能正确加载现有命令
- [ ] 编辑器能正确保存修改
- [ ] 原始命令文本正确更新
- [ ] 卡片标题正确显示
- [ ] 验证逻辑正确（保存按钮状态）

#### UI/UX 检查
- [ ] 显示加载状态
- [ ] 显示错误状态 + 重试按钮
- [ ] 显示空状态（如果适用）
- [ ] 字段标签清晰易懂
- [ ] 输入框有合理的 placeholder
- [ ] 必填字段有标记

#### 代码质量
- [ ] 没有重复代码（与现有编辑器对比）
- [ ] 使用了类型定义
- [ ] 变量命名清晰
- [ ] 逻辑清晰易懂
- [ ] 有必要的注释

---

## 常见问题解答

### Q1: 什么时候使用 FormEditor？

**A:** 当命令参数简单、不需要复杂的自定义 UI 时。

**使用 FormEditor：**
- ✅ 只有文本、数字、下拉选择等简单字段
- ✅ 不需要 Database API
- ✅ 不需要复杂的级联逻辑

**不使用 FormEditor：**
- ❌ 需要从 Database API 动态加载选项
- ❌ 需要复杂的向量输入（使用 VectorInput）
- ❌ 需要复杂的级联逻辑（如 actorgroup 的模型选择）

### Q2: 如何判断是否需要 useAvailableIds？

**A:** 如果命令需要引用其他命令中定义的 ID，就使用。

**典型场景：**
- `actormotion` / `actorfacialmotion` 需要 actor ID（来自 `actorgroup`）
- `backgroundsetting` 需要 background ID（来自 `backgroundgroup`）
- `actorlayoutgroup` 需要 actor ID（来自 `actorgroup`）

### Q3: 什么时候需要显示 EmptyState？

**A:** 当命令强制依赖其他命令的数据时。

**示例：**
```typescript
const availableActorIds = useAvailableActorIds();

if (availableActorIds.length === 0) {
  return (
    <EmptyState 
      title="无可用角色" 
      description="请先添加 actorgroup 命令来定义角色"
    />
  );
}
```

**不要降级到文本输入！** 这会让用户输入错误的值。

### Q4: 如何处理条件参数？

**A:** 在 `handleFieldChange` 中根据条件添加/删除参数。

**关键点：**
1. 使用 `delete` 删除不需要的参数
2. 在保存时检测参数数量变化（触发完整重建）
3. 在渲染器中条件渲染
4. 在卡片标题中条件显示

参考 `TransitionEditor` 的实现。

### Q5: Group 编辑器的添加和编辑有什么区别？

**A:** 通过 `editingItemIndex` 区分：

- `editingItemIndex === -2` → 添加模式（新建项）
- `editingItemIndex >= 0` → 编辑模式（修改现有项）

```typescript
if (editingItemIndex === -2) {
  items.push(editingItemData);  // 添加
} else if (editingItemIndex >= 0) {
  items[editingItemIndex] = editingItemData;  // 更新
}
```

### Q6: 如何处理 JSON 转义？

**A:** 根据命令格式决定是否需要转义。

**需要转义（如 `camerasetting`）：**
```typescript
const escaped = JSON.stringify(data)
  .replace(/{/g, '\\{')
  .replace(/}/g, '\\}');
```

**不需要转义（如 `backgroundsetting`）：**
```typescript
const jsonString = JSON.stringify(data);  // 直接使用
```

**判断方法：** 查看原始命令文本中的格式。

### Q7: 如何复用类似的编辑器？

**A:** 创建通用编辑器 + 配置参数。

**示例：** `ActorResourceEditor` 复用于 `actormotion` 和 `actorfacialmotion`

```typescript
// ActorResourceEditor.tsx（通用）
interface ActorResourceEditorConfig {
  resourceType: 'character' | 'facial';
  resourceLabel: string;
  extraFields?: FormField[];
}

export const ActorResourceEditor: React.FC<Props & { config: Config }> = ({ 
  card, onChange, config 
}) => {
  // 通用逻辑
};

// ActorMotionEditor.tsx（特化）
export const ActorMotionEditor: React.FC<Props> = (props) => (
  <ActorResourceEditor 
    {...props} 
    config={{ resourceType: 'character', resourceLabel: '动作名称' }} 
  />
);
```

### Q8: 如何测试编辑器？

**A:** 手动测试流程：

1. **加载测试**
   - 打开示例文件（sample.txt）
   - 找到对应的命令
   - 点击编辑按钮
   - 确认参数正确显示

2. **编辑测试**
   - 修改各个参数
   - 点击保存
   - 确认卡片标题更新
   - 确认原始命令文本更新

3. **验证测试**
   - 清空必填字段
   - 确认保存按钮禁用
   - 填写必填字段
   - 确认保存按钮启用

4. **边界测试**
   - 测试空值
   - 测试极限值
   - 测试错误输入

5. **依赖测试**（如果适用）
   - 删除前置命令（如 actorgroup）
   - 确认显示 EmptyState
   - 重新添加前置命令
   - 确认恢复正常

---

## 附录

### 快速参考

#### 基础设施导入

```typescript
// Context
import { useEditorContext, useEditorCards, useSelectedCard } from '../../../context/EditorContext';

// Hooks
import { useResourceAPI, useMotions, useEnvironments, useModels, useCharacters } from '../../../hooks/useResourceAPI';
import { useAvailableActorIds, useAvailableBackgroundIds, isBackground2D } from '../../../hooks/useAvailableIds';

// Components
import { LoadingState, ErrorState, EmptyState, Vector2Input, Vector3Input } from '../../common';
import { FormEditor, FormSection, FormField } from '../FormEditor';
import { ParamCard, ParamRow, AddItemButton } from '../../renderers/ParamRow';
```

#### 常用代码片段

**加载资源：**
```typescript
const { data: resources, loading, error, reload } = useMotions('character');

if (loading) return <LoadingState message="加载资源中..." />;
if (error) return <ErrorState message="加载失败" details={error} onRetry={reload} />;
```

**检查依赖：**
```typescript
const availableIds = useAvailableActorIds();

if (availableIds.length === 0) {
  return <EmptyState title="无可用角色" description="请先添加 actorgroup 命令" />;
}
```

**实时验证：**
```typescript
useEffect(() => {
  const isValid = formData.params.id !== '' && formData.params.value > 0;
  onChange(formData, isValid);
}, [formData]);
```

**向量输入：**
```typescript
<Vector3Input
  value={position}
  onChange={setPosition}
  step={0.01}
  labels={{ x: 'X轴', y: 'Y轴', z: 'Z轴' }}
/>
```

---

## 总结

遵循本规范可以确保：

- ✅ **代码一致性** - 所有编辑器使用统一的实现模式
- ✅ **避免重复** - 最大化复用基础设施
- ✅ **用户体验** - 统一的 UI 和交互
- ✅ **易于维护** - 清晰的架构和文档
- ✅ **类型安全** - 完整的 TypeScript 类型定义

**记住核心原则：**
1. 不要重新发明轮子 - 使用现有的基础设施
2. 保持一致性 - 参考现有的最佳实践
3. 优先用户体验 - 清晰的提示和友好的错误处理

---

📝 **文档版本**: v1.0  
📅 **最后更新**: 2026-01-13  
👤 **维护者**: Editor2 开发团队
